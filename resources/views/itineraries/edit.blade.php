<x-app-layout>
    @php
        $fallbackCover = \App\Models\Itinerary::whereNotNull('cover_image_remote_url')
            ->whereKeyNot($itinerary->id)
            ->latest()
            ->first();
    @endphp
    <x-slot name="header">
        <section class="surface-panel-dark overflow-hidden p-8 sm:p-10">
            <div class="flex flex-wrap items-center justify-between gap-4">
                <div>
                    <p class="hero-kicker">Edit itinerary</p>
                    <h1 class="mt-4 text-5xl font-semibold leading-none text-white sm:text-6xl">Refine the details behind {{ $itinerary->title }}.</h1>
                    <p class="mt-5 max-w-2xl text-base leading-8 text-slate-300">
                        Update the trip story, dates, cover art, and visibility settings without losing the polished presentation.
                    </p>
                </div>
                <a href="{{ route('itineraries.show', $itinerary) }}" class="rounded-full border border-white/15 px-6 py-3 text-sm font-semibold text-white transition hover:bg-white/10">
                    Back to itinerary
                </a>
            </div>
        </section>
    ...
    <div class="grid gap-8 pb-12 lg:grid-cols-[1.15fr_0.85fr]">
        <section class="surface-panel p-6 sm:p-8">
            <form action="{{ route('itineraries.update', $itinerary) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                @csrf
                @method('PUT')

                <div>
                    <x-input-label for="title" :value="__('Trip title')" />
                    <x-text-input id="title" name="title" type="text" :value="old('title', $itinerary->title)" required autofocus />
                    <x-input-error :messages="$errors->get('title')" class="mt-2" />
                </div>

                <div>
                    <x-input-label for="description" :value="__('Description')" />
                    <textarea id="description" name="description" rows="5" class="w-full rounded-[24px] border border-slate-200 bg-white px-4 py-3 text-sm text-slate-900 shadow-sm transition placeholder:text-slate-400 focus:border-teal-500 focus:outline-none focus:ring-4 focus:ring-teal-500/10">{{ old('description', $itinerary->description) }}</textarea>
                ...
                <div class="grid gap-6 sm:grid-cols-2">
                    <div>
                        ...
                        <x-text-input id="start_date" name="start_date" type="date" :value="old('start_date', $itinerary->start_date->format('Y-m-d'))" required />
                        <x-input-error :messages="$errors->get('start_date')" class="mt-2" />
                    </div>
                    <div>
                        <x-input-label for="end_date" :value="__('End date')" />
                        <x-text-input id="end_date" name="end_date" type="date" :value="old('end_date', $itinerary->end_date->format('Y-m-d'))" required />
                ...
                <div>
                    ...
                    <div class="mt-3 overflow-hidden rounded-[24px] border border-slate-200">
                        <img src="{{ $itinerary->cover_image_url }}" alt="{{ $itinerary->title }}" class="h-56 w-full object-cover">
                    </div>
                    ...
                    <div class="mt-4 rounded-[28px] border border-slate-200 bg-slate-50/80 p-5">
                        <div class="flex flex-col gap-3 sm:flex-row">
                            <input
                                ...
                                placeholder="Search a place like Shibuya, Amalfi Coast, or Machu Picchu"
                            >
                        ...
                        <p id="cover-search-status" class="mt-3 text-sm text-slate-500">Pick a new real place photo or keep the current cover.</p>
                    ...
                    <label for="cover_image" class="mt-4 flex cursor-pointer flex-col items-center justify-center rounded-[28px] border border-dashed border-slate-300 bg-slate-50 px-6 py-10 text-center transition hover:border-teal-500 hover:bg-teal-50/40">
                        ...
                        <span class="mt-4 text-base font-semibold text-slate-900">Upload a new image if you want to replace the current cover manually.</span>
                        <span class="mt-2 text-sm text-slate-500">A file upload overrides any selected API photo.</span>
                    ...
                    <div id="image-preview" class="mt-4 hidden overflow-hidden rounded-[24px] border border-slate-200">
                        <img id="preview-img" src="#" alt="New cover preview" class="h-56 w-full object-cover">
                    </div>
                    <p id="selected-image-credit" class="mt-3 hidden text-sm leading-6 text-slate-500"></p>

                    <input type="hidden" name="selected_cover_image_provider" id="selected_cover_image_provider" value="{{ old('selected_cover_image_provider', $itinerary->cover_image_provider) }}">
                    <input type="hidden" name="selected_cover_image_url" id="selected_cover_image_url" value="{{ old('selected_cover_image_url', $itinerary->cover_image_remote_url) }}">
                    <input type="hidden" name="selected_cover_image_author_name" id="selected_cover_image_author_name" value="{{ old('selected_cover_image_author_name', $itinerary->cover_image_author_name) }}">
                    <input type="hidden" name="selected_cover_image_author_url" id="selected_cover_image_author_url" value="{{ old('selected_cover_image_author_url', $itinerary->cover_image_author_url) }}">
                    <input type="hidden" name="selected_cover_image_source_url" id="selected_cover_image_source_url" value="{{ old('selected_cover_image_source_url', $itinerary->cover_image_source_url) }}">
                ...
                <label for="is_public" class="flex items-start gap-4 rounded-[24px] border border-slate-200 bg-slate-50/80 px-5 py-4">
                    <input id="is_public" type="checkbox" class="mt-1 rounded border-slate-300 text-teal-700 focus:ring-teal-500" name="is_public" value="1" {{ old('is_public', $itinerary->is_public) ? 'checked' : '' }}>
                    <span>
                        <span class="block text-sm font-semibold text-slate-900">Enable public sharing</span>
                        <span class="mt-1 block text-sm text-slate-600">Public itineraries can be opened through links and QR codes.</span>
                    </span>
                </label>

                <div class="flex flex-wrap justify-end gap-3 border-t border-slate-200 pt-6">
                    <a href="{{ route('itineraries.show', $itinerary) }}" class="button-ghost">Cancel</a>
            ...
            <section class="surface-panel overflow-hidden">
                ...
                <div class="p-6">
                    ...
                    <p class="mt-3 text-sm leading-7 text-slate-600">
                        If the uploaded cover is removed or unavailable, the itinerary will still present a place-relevant illustration.
                    </p>
                </div>
            </section>

            <section class="surface-panel p-6">
                <h3 class="text-3xl font-semibold text-slate-950">Editing reminders</h3>
                <div class="mt-5 space-y-4 text-sm leading-7 text-slate-600">
                    <div class="rounded-[24px] bg-slate-50 px-4 py-3">Update the description if the tone or purpose of the trip has changed.</div>
                    <div class="rounded-[24px] bg-slate-50 px-4 py-3">Adjust dates before adding new destination timings to keep the schedule clean.</div>
                    <div class="rounded-[24px] bg-slate-50 px-4 py-3">Use public sharing only for trip plans that are ready to be seen outside your account.</div>
                </div>
            </section>
        </aside>
    </div>
